...
var
  ...
  procedure scroll_up;
  var
    src,dest: tSCREEN_MEM_PTR;
  begin
    Move(text_screen_shadow[MaxCol*2],text_screen_shadow[0],(MaxLn-1)*MaxCol*2);
    FillWord(text_screen_shadow[(MaxLn-1)*MaxCol*2],MaxCol DIV 2,$0700);
  end;
...
begin
  ...
  For temp := 1 to Length(str) do
    If (str[temp] <> '~') then
      begin
        text_screen_shadow[(posx-1+(posy-1)*MaxCol) SHL 1] := Byte(str[temp]);
        text_screen_shadow[(posx-1+(posy-1)*MaxCol) SHL 1+1] := attr;
