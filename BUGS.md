...
- **Fix applied** (`src/a2t.c`): Added logic to preserve target frequency from previous portamento when `has_carry=true` but `has_note=false`.
- **Resolved module**: `fm-troni` — 1,010 → **0** (verified at 50,622 frames).
- **REGRESSION**: The fix broke 8 modules that previously had fewer diffs. The carry-over logic now incorrectly preserves target frequency in cases where Pascal does not. Regressed modules:

| Module | Before | After |
|--------|--------|-------|
| nightdrv | 5,438 | 193,394 |
| aquarius | 1,946 | 73,361 |
| crackit | 15,122 | 430,208 |
| drgwrrtt | 352 | 183,325 |
| old_002 | 2,942 | 596,344 |
| 4xmisste | 614 | 284,492 |
| yellowwe | 272 | 15,392 |
| KULJE_V4 | 675,825 | 969,742 |

- **Also resolved** (previously FRAME-DIFF, now PASS): funland (383→0), goldebel (383→0), lemmings (48,008→0), lostcaus (26,007→0), psg (13,610→0), opl303 (25,346→0), pink (2,297→0), spacediv (50,287→0).
- **Investigation needed**: Determine the exact condition under which Pascal preserves vs resets the portamento target frequency during carry-over. The current fix is too aggressive — it preserves the target frequency in all carry-over cases, but Pascal appears to have more nuanced logic.
...
## Test Results Summary (2026-05-20)

| Module Set | Total | PASS | FRAME-DIFF | Notes |
|---|---|---|---|---|---|---|
| encore | 10 | 10 | 0 | All PASS. Bug 10 resolved amegas (437K→0). Bug 1 fixed remaining. |
| hydra | 2 | 1 | 1 | `crackit`: REGRESSION after Bug 19 fix (15,122→430,208). `intrcoop`: PASS. |
| dretz | 11 | 11 | 0 | All PASS at 100k frames |
| o2star | 15 | 15 | 0 | All PASS |
| televics | 36 | 34 | 2 | `4xmisste`: REGRESSION (614→284,492). `yellowwe`: REGRESSION (272→15,392). Rest PASS. |
| brendan | ~70 | ~67 | 3 | `funland`, `goldebel`, `lemmings`, `lostcaus` now PASS (Bug 19 fix). |
| diodema | 23 | 22 | 1 | `aquarius`: REGRESSION (1,946→73,361). `psg` now PASS (13,610→0). |
| mlf | 14 | 11 | 3 | `fm-troni` PASS (1,010→0, Bug 19 fix). `opl303`, `pink`, `spacediv` PASS. `old_002`: REGRESSION (2,942→596,344). |
| nula | 20 | 20 | 0 | All PASS |
...
- `src/a2t.c:1557-1560` — C's TonePortamento target frequency calculation (Bug 19 fix: added carry-over preservation — REGRESSION)
...
1. **Bug 19 (TonePortamento carry-over regression)** — URGENT. The fix for `fm-troni` broke 8 other modules. Need to find the exact condition under which Pascal preserves vs resets the portamento target frequency during carry-over. Likely involves checking whether the current row has a portamento effect definition vs just a note.
2. **Bug 11 (keyoff_loop state divergence)** — investigate why Pascal's `keyoff_loop[13..14]` is `true` at frame 161+ for `adr1ft.a2m` while C's is `false`. No X1 effects (`ef_ex_cmd_MKOffLoopEn`) found in pattern data for channels 12-13. Both implementations initialize `keyoff_loop=FALSE` and only set it via X1 effect. Benign: zero shadow register or audio diffs, strictly MB (macro state) dump difference. 118,703 diff lines across 3,514 frames (all MB-only). Also affects `4xmisste` (25,362 MB), `5ontelev` (2,304 MB), `glass` (12,240 MB), `xmission` (20,754 MB), `zaxxon` (2,304 MB).
3. **Bug 9 (arpeggio state divergence)** — investigate exact state transition sequence between C and Pascal for `rbfactry` frames 1377→1378. Compare Pascal's `arpgg_cleanup` (`a2player.pas:1361-1374`) and effect carry-over logic (`a2player.pas:1494-1535`) against C's `play_line_arpgg_cleanup_pascal` and `process_effects_slot_body`.
4. **Bug 7 (instrument data truncated)** — fix C's loader to read all 255 instrument slots from the file instead of only `count` entries. This would resolve the remaining MV/CV diffs in `o2ghosts`, `sparkplg`, `sweetsin`.
5. ~~Fix `e2_vslide_type` initialization~~ — confirmed NOT A BUG.
6. ~~**Bug 3 (TonePortamento on note=0)** — FIXED 2026-05-15.~~ Resolved 7 modules.
7. ~~**Bug 6 (NULL instrument in volume functions)** — FIXED 2026-05-16.~~ Resolved `mechwar` (25,624 → 0).
8. ~~**Bug 8 (Pascal SetInsVolume bounds guard)** — FIXED 2026-05-16.~~ Resolved `dream7mx` (0x14c divergence eliminated).
9. ~~**Bug 10 (C ef_SetInsVolume/ef_ForceInsVolume is_data_empty guard)** — FIXED 2026-05-17.~~ Resolved `amegas` (437,366 → 0).
10. ~~**Bug 12 (TonePortamento effect_table cleared on new note with val=0)** — FIXED 2026-05-18.~~ Resolved `limitbrk` (310,177 → 0).
11. ~~**Bug 13 (v5-8 loader missing ManualFSlide→FineTune conversion)** — FIXED 2026-05-18.~~ Resolved `old_002` (185,144 → 2,942, 98.4% reduction).
