...
| adr1ft (modules/diodema) | 21498 | 40172 | FRAME-DIFF | `run_one_test.sh modules/diodema/adr1ft.a2m` with default `MAX_FRAMES=15000` (2026-05-13): **40172** diff lines from IRQ **161**, bank **1** only. FMREG macro (inst 6, len=51) applied to ch13/ch14 at row 4. C applies ALL macro cell columns because `dis_fmreg_cols=0x00000000` (raw file data all zeros = no columns disabled). Pascal's registers stay constant — suggesting Pascal either doesn't allocate the FMREG table for inst 6, or uses inverted disabled_fmregs logic (0=disabled, 1=enabled). Context trace at IRQ 161 confirmed: `fmreg_pos=1`, `active_cell[0]: dur=1 volM=39 volC=48`, `dis=0x00000000`. C applies volume/AM/VIB/ADSR from cells each macro tick, causing all bank 1 registers to slide. Next: compare Pascal's `instr_macros` allocation for inst 6 and `dis_fmregs` bit semantics. |
...
| no72 | 15000 | 59810 | FRAME-DIFF | `run_one_test.sh modules/no72.a2m` with default `MAX_FRAMES=15000` (2026-05-13): **59810** diff lines from frame **0** across ALL registers — complete INIT state divergence. C and Pascal produce entirely different register states from the first frame onwards. Pre-existing issue (not caused by current changes). Previous `PASS` entry was inaccurate — the `fmreg_infer_length_from_cells` fix for `length=0`/`header_links` was discussed but the inference code remains active. |
