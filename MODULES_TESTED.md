...
| badapple | 100000 | 71752 | FRAME-DIFF | **`A2M_DUMP_CONTEXT`** at IRQ **14874–14880** (`make test DUMP_CONTEXT=1`, stderr → e.g. `test/badapple_ctx.log`): divergence aligns with **pattern 53**, **order 10**, **row 112→113** boundary (**ticks** **6199→6201**). **Channel 4** (4‑op **high**, paired low **5**): **`freq_table[4]=0x2599`**, **`ftune_table[4]=1`** (**Extended2** **FineTuneUp** **`0x41`** on **`effect_table[0][4]`**), **`macro_table[4].vib_freq=0x0599`**, **`vib_table=0`** (no macro vibrato path). **`vibr_table` / `trem_table`** all zero — not pattern vibrato/trem. Pascal expects **`0x259a`** (F-num low **`0x9a`** vs C **`0x99`**) — **single‑step** pitch delta on **`nFreq`/fine‑tune family**, not macro TL. **`macro_poll_proc`**: fixed typo **`&` → `&&`** in macro vibrato **keyoff_pos** guard (**`a2t.c`**) for Pascal‑style boolean logic (did **not** shrink **`diff -u`** in a quick retest). Next suspects: **`ins_parameter(…,12)` vs `get_instr_fine_tune`**, **`arpgg_table` slot‑pair vs Pascal unified `arpgg_table[chan]` cleanup**, or row **113** cell ordering vs **`play_line`** loops. |
